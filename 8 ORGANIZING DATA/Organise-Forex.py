...
for i in range(ForexDay.shape[0]):
    ...
    for j in range(ForexDayRate.shape[0]):
        ...
        nMoneyValue = str(round(ForexDayRate['Money'][j], 2))
        sCodeIn = ForexDayRate['PCurrency'][j]
        sCodeOut = ForexDayRate['Currency'][j]

    updateSQL = (
        f'UPDATE MoneyData SET Date="{sDay}", '
        f'Money={nMoneyValue}, Currency="{sCodeOut}" '
        f'WHERE Bag={sBag} AND Currency="{sCodeIn}";'
    )
    updateSQL = re.sub("\s\s+", " ", updateSQL)
    ...
    t += 1
    print('Trade :', t, sDay, sCodeOut, nMoneyValue)

    insertSQL = f"""
    INSERT INTO TransactionData (
        RowID, Bag, Date, Money, Currency
    )
    SELECT {t} AS RowID,
           Bag,
           Date,
    ...
    insertSQL = re.sub("\s\s+", " ", insertSQL)
