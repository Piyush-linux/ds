...
sSQL = "select distinct A.CodeIn from Assess_Forex as A;"
...
for c in range(CodeData.shape[0]):
    ...
    sSQL = (
        ...
        "A.Date, "
        "A.CodeIn, "
        "B.Country as CountryIn, "
        "B.Currency as CurrencyNameIn, "
        "A.CodeOut, "
        "C.Country as CountryOut, "
        "C.Currency as CurrencyNameOut, "
        "A.Rate "
        "from Assess_Forex as A "
        "JOIN Assess_Country as B "
        "ON A.CodeIn = B.CurrencyCode "
        "JOIN Assess_Country as C "
        "ON A.CodeOut = C.CurrencyCode "
        "WHERE A.CodeIn = '" + CodeData['CodeIn'][c] + "';"
    )
