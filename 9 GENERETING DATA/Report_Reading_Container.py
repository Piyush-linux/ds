...
def plot_embedding(X, title=None):
    """Plot 2D embedding with digit labels and image thumbnails."""
    ...
    if hasattr(offsetbox, 'AnnotationBbox'):
        shown_images = np.array([[1.0, 1.0]])  # Something large
...
plot_embedding(
    X_lda,
    "Linear Discriminant projection of the digits (time %.2fs)" % (time() - t0)
)
...
plot_embedding(
    X_lle,
    "Locally Linear Embedding of the digits (time %.2fs)" % (time() - t0)
)
